...
def get_ramp_multiplier(ramp_option, T, wp_final_multiplier=1.0):
    ...
    """
    if ramp_option == RampOptions.RAMP_CONSTANT:
        wpm = np.ones(T)
    elif ramp_option == RampOptions.RAMP_LINEAR:
        wpm = (np.arange(T, dtype=np.float32) + 1) / T
    elif ramp_option == RampOptions.RAMP_QUADRATIC:
        wpm = ((np.arange(T, dtype=np.float32) + 1) / T) ** 2
    elif ramp_option == RampOptions.RAMP_FINAL_ONLY:
        wpm = np.zeros(T)
        wpm[T-1] = 1.0
    else:
