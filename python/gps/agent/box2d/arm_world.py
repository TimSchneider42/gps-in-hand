...
class ArmWorld(Framework):
    ...
    def reset_world(self, x0):
        ...
        self.set_joint_angles(self.body1, self.body2, x0["angles"][0], x0["angles"][1])
        ...
        self.joint1.motorSpeed = x0["vel"][0]
        self.joint2.motorSpeed = x0["vel"][0]

    def get_state(self):
        """Retrieves the state of the point mass"""
        state = {"angles": np.array([self.joint1.angle,
                                         self.joint2.angle]),
                 "vel": np.array([self.joint1.speed,
                                             self.joint2.speed]),
                 "tracking": np.append(np.array(self.body2.position), [0])}

        return state

    @property
    def state_dimensions(self):
        return {"angles": 2, "vel": 2, "tracking": 3}
