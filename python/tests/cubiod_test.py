...
from allegro_pybullet.simulation_body.allegro_hand import AllegroFingerType, AllegroFingerJointType, AllegroRightHand
...
pc.configure_debug_visualizer(pybullet.COV_ENABLE_MOUSE_PICKING, True)
...
pc.gravity[2] = 9.81
pc.add_body(
    URDFBody("../gps/agent/allegro_pybullet/allegro_pybullet_data/cuboid_description.urdf", base_position=[0.2, 0.3, 0.4]))
...
for i in range(5000):
    for ft in AllegroFingerType:
        for jt in AllegroFingerJointType:
            hand.fingers[ft].joints[jt].torque_force = random.uniform(-0.1, 0.1)
    pc.step_simulation()
    sleep(pc.time_step)
